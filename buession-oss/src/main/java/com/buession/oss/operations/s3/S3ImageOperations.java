 ...
 * @author Yong.Teng
 */
public class AliCloudImageOperations extends AbstractS3Operations implements ImageOperations {
	...
	@Override
	public File resize(String bucketName, String path, int width, int height, boolean limit, String padColor)
			throws OSSException{
		Assert.isBlank(bucketName, "Bucket name cloud not be null or empty.");
		Assert.isBlank(path, "Resize image path cloud not be null or empty.");

		final String style =
				ObjectBuilder.ObjectResizeBuilder.getInstance().width(width).height(height).limit(limit)
						.padColor(padColor).build();
		return doProcess(bucketName, path, style);
	}
	...
	@Override
	public File resize(String bucketName, String path, int width, int height, int l, int s, String padColor)
			throws OSSException{
		Assert.isBlank(bucketName, "Bucket name cloud not be null or empty.");
		Assert.isBlank(path, "Resize image path cloud not be null or empty.");

		final String style =
				ObjectBuilder.ObjectResizeBuilder.getInstance().width(width).height(height).l(l).s(s).padColor(padColor)
						.build();
		return doProcess(bucketName, path, style);
	}
	...
	public File resize(String bucketName, String path, int width, int height, int l, int s, boolean limit)
		...
		final String style =
				ObjectBuilder.ObjectResizeBuilder.getInstance().width(width).height(height).l(l).s(s).limit(limit)
						.build();
		return doProcess(bucketName, path, style);
	}
	...
	@Override
	public File resize(String bucketName, String path, ImageResizeMode mode, int width, int height, String padColor)
			throws OSSException{
		Assert.isBlank(bucketName, "Bucket name cloud not be null or empty.");
		Assert.isBlank(path, "Resize image path cloud not be null or empty.");
		Assert.isNull(mode, "Resize mode cloud not be null or empty.");
		...
		final String style =
				ObjectBuilder.ObjectResizeBuilder.getInstance().mode(mode).width(width).height(height)
						.padColor(padColor).build();
		return doProcess(bucketName, path, style);
	}
	...
	public File resize(String bucketName, String path, ImageResizeMode mode, int width, int height, boolean limit)
		...
		final String style =
				ObjectBuilder.ObjectResizeBuilder.getInstance().mode(mode).width(width).height(height).limit(limit)
						.build();
		return doProcess(bucketName, path, style);
	}
	...
	public File resize(String bucketName, String path, ImageResizeMode mode, int width, int height, int l, int s)
		...
		final String style =
				ObjectBuilder.ObjectResizeBuilder.getInstance().mode(mode).width(width).height(height).l(l).s(s)
						.build();
		return doProcess(bucketName, path, style);
	}
	...
	public File resize(String bucketName, String path, ImageResizeMode mode, int width, int height, int l, int s,
		...
		final String style =
				ObjectBuilder.ObjectResizeBuilder.getInstance().mode(mode).width(width).height(height).l(l).s(s)
						.limit(limit).padColor(padColor).build();
		return doProcess(bucketName, path, style);
	}
