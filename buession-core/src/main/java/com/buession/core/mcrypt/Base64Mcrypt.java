...
public final class Base64Mcrypt extends AbstractMcrypt {
    ...
    public String encode(final Object object){
        Assert.isNull(object, "Base64Mcrypt encode object could not be null.");

        String s = object2String(object);

        logger.debug("Base64Mcrypt encode string <{}> by algo <base64>, salt <{}>", object, getSalt());

        return Base64.encodeBase64String((s + (getSalt() == null ? "" : getSalt())).getBytes(getCharset()));
    }
    ...
    public String decode(final CharSequence cs){
        Assert.isNull(cs, "Base64Mcrypt decode object could not be null.");
        logger.debug("Base64Mcrypt decode string <{}> by algo <base64>, salt <{}>", cs, getSalt());
