...
public class CacheDatabaseResolver extends DatabaseResolver {
	...
	private final static int CACHE_INITIAL_CAPACITY = 128;

	private final static float CACHE_LOAD_FACTOR = 0.8F;

	private final static Map<String, Country> COUNTRY_CACHE_DATA = new ConcurrentHashMap<>(CACHE_INITIAL_CAPACITY,
			CACHE_LOAD_FACTOR);

	private final static Map<String, District> DISTRICT_CACHE_DATA = new ConcurrentHashMap<>(CACHE_INITIAL_CAPACITY,
			CACHE_LOAD_FACTOR);

	private final static Map<String, Location> LOCATION_CACHE_DATA = new ConcurrentHashMap<>(CACHE_INITIAL_CAPACITY,
			CACHE_LOAD_FACTOR);
