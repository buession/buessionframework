...
public final class LettuceClusterGeoOperations extends AbstractGeoOperations<LettuceClusterClient> {
	...
	public Long geoAdd(final byte[] key, final byte[] member, final double longitude, final double latitude) {
		...
		if(isPipeline()){
			return new LettuceClusterPipelineCommand<>(client, Command.GEOADD,
					(cmd)->cmd.geoadd(key, longitude, latitude, member), (v)->v)
					.run(args);
		}else if(isTransaction()){
			return new LettuceClusterTransactionCommand<>(client, Command.GEOADD,
					(cmd)->cmd.geoadd(key, longitude, latitude, member), (v)->v)
					.run(args);
		}else{
			return new LettuceClusterCommand<>(client, Command.GEOADD,
					(cmd)->cmd.geoadd(key, longitude, latitude, member), (v)->v)
					.run(args);
		}
	}
	...
	public List<Geo> geoPos(final byte[] key, final byte[]... members) {
		...
		if(isPipeline()){
			return new LettuceClusterPipelineCommand<>(client, Command.GEOPOS, (cmd)->cmd.geopos(key, members),
					listGeoCoordinateConverter)
					.run(args);
		}else if(isTransaction()){
			return new LettuceClusterTransactionCommand<>(client, Command.GEOPOS, (cmd)->cmd.geopos(key, members),
					listGeoCoordinateConverter)
					.run(args);
		}else{
			return new LettuceClusterCommand<>(client, Command.GEOPOS, (cmd)->cmd.geopos(key, members),
					listGeoCoordinateConverter)
					.run(args);
		}
	}
	...
	public List<GeoRadius> geoRadius(final byte[] key, final double longitude, final double latitude,
		...
		if(isPipeline()){
			return new LettuceClusterPipelineCommand<>(client, Command.GEORADIUS,
					(cmd)->cmd.georadius(key, longitude, latitude, radius, geoArgsUnit),
					setListGeoRadiusGeneralResultConverter)
					.run(args);
		}else if(isTransaction()){
			return new LettuceClusterTransactionCommand<>(client, Command.GEORADIUS,
					(cmd)->cmd.georadius(key, longitude, latitude, radius, geoArgsUnit),
					setListGeoRadiusGeneralResultConverter)
					.run(args);
		}else{
			return new LettuceClusterCommand<>(client, Command.GEORADIUS,
					(cmd)->cmd.georadius(key, longitude, latitude, radius, geoArgsUnit),
					setListGeoRadiusGeneralResultConverter)
					.run(args);
		}
	...
	public List<GeoRadius> geoRadiusByMember(final byte[] key, final byte[] member, final double radius,
		...
		if(isPipeline()){
			return new LettuceClusterPipelineCommand<>(client, Command.GEORADIUSBYMEMBER,
					(cmd)->cmd.georadiusbymember(key, member, radius, geoArgsUnit),
					setListGeoRadiusGeneralResultConverter)
					.run(args);
		}else if(isTransaction()){
			return new LettuceClusterTransactionCommand<>(client, Command.GEORADIUSBYMEMBER,
					(cmd)->cmd.georadiusbymember(key, member, radius, geoArgsUnit),
					setListGeoRadiusGeneralResultConverter)
					.run(args);
		}else{
			return new LettuceClusterCommand<>(client, Command.GEORADIUSBYMEMBER,
					(cmd)->cmd.georadiusbymember(key, member, radius, geoArgsUnit),
					setListGeoRadiusGeneralResultConverter)
					.run(args);
		}
	...
	public List<GeoRadius> geoSearch(final byte[] key, final GeoSearchArgument geoSearchArgument) {
		...
		if(isPipeline()){
			return new LettuceClusterPipelineCommand<>(client, Command.GEOSEARCH, (cmd)->cmd.geosearch(key, geoRef,
					geoPredicate, geoArgs), listGeoRadiusResponseConverter)
					.run(args);
		}else if(isTransaction()){
			return new LettuceClusterTransactionCommand<>(client, Command.GEOSEARCH, (cmd)->cmd.geosearch(key, geoRef,
					geoPredicate, geoArgs), listGeoRadiusResponseConverter)
					.run(args);
		}else{
			return new LettuceClusterCommand<>(client, Command.GEOSEARCH, (cmd)->cmd.geosearch(key, geoRef,
					geoPredicate, geoArgs), listGeoRadiusResponseConverter)
					.run(args);
		}
	}
	...
	public Long geoSearchStore(final byte[] destKey, final byte[] key,
		...
		if(isPipeline()){
			return new LettuceClusterPipelineCommand<>(client, Command.GEOSEARCHSTORE,
					(cmd)->cmd.geosearchstore(destKey, key, geoRef, geoPredicate, geoArgs, storeDist), (v)->v)
					.run(args);
		}else if(isTransaction()){
			return new LettuceClusterTransactionCommand<>(client, Command.GEOSEARCHSTORE,
					(cmd)->cmd.geosearchstore(destKey, key, geoRef, geoPredicate, geoArgs, storeDist), (v)->v)
					.run(args);
		}else{
			return new LettuceClusterCommand<>(client, Command.GEOSEARCHSTORE,
					(cmd)->cmd.geosearchstore(destKey, key, geoRef,
	...
	private Long geoAdd(final byte[] key, final GeoValue<byte[]>[] geoValues, final CommandArguments args) {
		if(isPipeline()){
			return new LettuceClusterPipelineCommand<>(client, Command.GEOADD, (cmd)->cmd.geoadd(key, geoValues),
					(v)->v)
					.run(args);
		}else if(isTransaction()){
			return new LettuceClusterTransactionCommand<>(client, Command.GEOADD, (cmd)->cmd.geoadd(key, geoValues),
					(v)->v)
					.run(args);
		}else{
			return new LettuceClusterCommand<>(client, Command.GEOADD, (cmd)->cmd.geoadd(key, geoValues), (v)->v)
					.run(args);
		}
	}

	private Long geoAdd(final byte[] key, final GeoValue<byte[]>[] geoValues,
		...
		if(isPipeline()){
			return new LettuceClusterPipelineCommand<>(client, Command.GEOADD,
					(cmd)->cmd.geoadd(key, geoAddArgs, geoValues), (v)->v)
					.run(args);
		}else if(isTransaction()){
			return new LettuceClusterTransactionCommand<>(client, Command.GEOADD,
					(cmd)->cmd.geoadd(key, geoAddArgs, geoValues), (v)->v)
	...
	private Double geoDist(final byte[] key, final byte[] member1, final byte[] member2, final GeoArgs.Unit unit,
						   final CommandArguments args) {
		if(isPipeline()){
			return new LettuceClusterPipelineCommand<>(client, Command.GEODIST,
					(cmd)->cmd.geodist(key, member1, member2, unit), (v)->v)
					.run(args);
		}else if(isTransaction()){
			return new LettuceClusterTransactionCommand<>(client, Command.GEODIST,
					(cmd)->cmd.geodist(key, member1, member2, unit), (v)->v)
					.run(args);
		}else{
			return new LettuceClusterCommand<>(client, Command.GEODIST, (cmd)->cmd.geodist(key, member1, member2, unit),
					(v)->v)
					.run(args);
		}
	}
	...
	private <V> List<V> geoHash(final byte[] key, final byte[][] members,
		...
		if(isPipeline()){
			return new LettuceClusterPipelineCommand<>(client, Command.GEOHASH, (cmd)->cmd.geohash(key, members),
					converter)
					.run(args);
		}else if(isTransaction()){
			return new LettuceClusterTransactionCommand<>(client, Command.GEOHASH, (cmd)->cmd.geohash(key, members),
					converter)
					.run(args);
		}else{
			return new LettuceClusterCommand<>(client, Command.GEOHASH, (cmd)->cmd.geohash(key, members), converter)
					.run(args);
		}
	}
