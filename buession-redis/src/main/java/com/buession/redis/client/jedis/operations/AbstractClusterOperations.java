...
public abstract class AbstractClusterOperations<C extends JedisRedisClient> extends AbstractJedisRedisOperations<C>
	...
	public Status readOnly() {
		return JedisCommandBuilder.<Status, Status>newBuilder(client, Command.CLUSTER, SubCommand.READONLY)
				.converter((v)->v).run();
	}

	@Override
	public Status readWrite() {
		return JedisCommandBuilder.<Status, Status>newBuilder(client, Command.CLUSTER, SubCommand.READWRITE)
				.converter((v)->v).run();
	}
