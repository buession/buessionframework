...
public final class JedisBitMapOperations extends AbstractBitMapOperations<JedisStandaloneClient> {
	...
	public Long bitCount(final String key, final long start, final long end, final BitType bitType) {
		...
		if(isPipeline()){
			return new JedisPipelineCommand<>(client, Command.BITCOUNT, (cmd)->cmd.bitcount(key, start, end, option),
					(v)->v)
					.run(args);
		}else if(isTransaction()){
			return new JedisTransactionCommand<>(client, Command.BITCOUNT, (cmd)->cmd.bitcount(key, start, end, option),
					(v)->v)
					.run(args);
		}else{
	...
	public List<Long> bitField(final String key) {
		...
		if(isPipeline()){
			return new JedisPipelineCommand<>(client, Command.BITFIELD, (cmd)->cmd.bitfield(key), (v)->v)
					.run(args);
		}else if(isTransaction()){
			return new JedisTransactionCommand<>(client, Command.BITFIELD, (cmd)->cmd.bitfield(key), (v)->v)
					.run(args);
		}else{
			return new JedisCommand<>(client, Command.BITFIELD, (cmd)->cmd.bitfield(key), (v)->v)
					.run(args);
		}
	}
	...
	public List<Long> bitField(final String key, final BitFieldArgument argument) {
		...
		if(isPipeline()){
			return new JedisPipelineCommand<>(client, Command.BITFIELD, (cmd)->cmd.bitfield(key, arguments), (v)->v)
					.run(args);
		}else if(isTransaction()){
			return new JedisTransactionCommand<>(client, Command.BITFIELD, (cmd)->cmd.bitfield(key, arguments), (v)->v)
					.run(args);
		}else{
	...
	public List<Long> bitFieldRo(final String key) {
		...
		if(isPipeline()){
			return new JedisPipelineCommand<>(client, Command.BITFIELD_RO, (cmd)->cmd.bitfieldReadonly(key), (v)->v)
					.run(args);
		}else if(isTransaction()){
			return new JedisTransactionCommand<>(client, Command.BITFIELD_RO, (cmd)->cmd.bitfieldReadonly(key), (v)->v)
					.run(args);
		}else{
			return new JedisCommand<>(client, Command.BITFIELD_RO, (cmd)->cmd.bitfieldReadonly(key), (v)->v)
					.run(args);
		}
	}
	...
	public List<Long> bitFieldRo(final String key, final BitFieldRoArgument argument) {
		...
		if(isPipeline()){
			return new JedisPipelineCommand<>(client, Command.BITFIELD_RO, (cmd)->cmd.bitfieldReadonly(key, arguments),
					(v)->v)
					.run(args);
		}else if(isTransaction()){
			...
			return new JedisCommand<>(client, Command.BITFIELD_RO, (cmd)->cmd.bitfieldReadonly(key, arguments), (v)->v)
					.run(args);
		}
	}
	...
	public Boolean setBit(final String key, final long offset, final boolean value) {
		...
		if(isPipeline()){
			return new JedisPipelineCommand<>(client, Command.SETBIT, (cmd)->cmd.setbit(key, offset, value), (v)->v)
					.run(args);
		}else if(isTransaction()){
			return new JedisTransactionCommand<>(client, Command.SETBIT, (cmd)->cmd.setbit(key, offset, value), (v)->v)
					.run(args);
		}else{
	...
	private Long bitPos(final String key, final boolean value, final BitPosParams bitPosParams,
		...
		if(isPipeline()){
			return new JedisPipelineCommand<>(client, Command.BITPOS, (cmd)->cmd.bitpos(key, value, bitPosParams),
					(v)->v)
					.run(args);
		}else if(isTransaction()){
			return new JedisTransactionCommand<>(client, Command.BITPOS, (cmd)->cmd.bitpos(key, value, bitPosParams),
					(v)->v)
					.run(args);
		}else{
			return new JedisCommand<>(client, Command.BITPOS, (cmd)->cmd.bitpos(key, value, bitPosParams), (v)->v)
					.run(args);
		}
	}
