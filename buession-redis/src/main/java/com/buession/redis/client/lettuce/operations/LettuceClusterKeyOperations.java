...
public final class LettuceClusterKeyOperations extends AbstractKeyOperations<LettuceClusterClient> {
	...
	public Set<String> keys(final String pattern) {
		...
		if(isPipeline()){
			...
			return new LettuceClusterCommand<>(client, ProtocolCommand.KEYS, (cmd)->cmd.keys(pattern),
					binaryToStringListSetConverter)
					.run(args);
		}
	}
	...
	public Set<byte[]> keys(final byte[] pattern) {
		...
		if(isPipeline()){
			return new LettuceClusterPipelineCommand<>(client, ProtocolCommand.KEYS, (cmd)->cmd.keys(sPattern),
					converter)
					.run(args);
		}else if(isTransaction()){
			return new LettuceClusterTransactionCommand<>(client, ProtocolCommand.KEYS, (cmd)->cmd.keys(sPattern),
					converter)
					.run(args);
		}else{
			return new LettuceClusterCommand<>(client, ProtocolCommand.KEYS, (cmd)->cmd.keys(sPattern), converter)
					.run(args);
		}
	}
