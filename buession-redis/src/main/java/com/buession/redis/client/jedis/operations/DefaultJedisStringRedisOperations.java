 ...
 * @author Yong.Teng
 */
public class DefaultJedisStringRedisOperations<C extends JedisCommands> extends AbstractJedisRedisOperations implements JedisStringRedisOperations {

	public DefaultJedisStringRedisOperations(final JedisRedisClient client){
		super(client);
	}
	...
	public Status setNx(final String key, final String value){
		...
		if(isTransaction()){
			...
			return execute((C cmd)->statusForBool(isTransaction() ? getTransaction().setnx(key, value).get() > 0 :
					cmd.setnx(key, value) > 0), ProtocolCommand.SETNX, args);
		}
	}
			...
			public Status execute(C cmd){
				...
				if(isTransaction()){
					...
					if(cmd instanceof Jedis){
						ret = ((Jedis) cmd).mset(temp.stream().toArray(String[]::new));
					}else{
		...
		return execute(new Executor<C, Status>() {
			...
			public Status execute(C cmd){
				...
				if(isTransaction()){
					...
					if(cmd instanceof Jedis){
						ret = ((Jedis) cmd).msetnx(temp.stream().toArray(String[]::new));
					}else{
	...
	public String substr(final String key, final int start, final int end){
		...
		if(isTransaction()){
			return execute((C cmd)->getTransaction().substr(key, start, end).get(), ProtocolCommand.SUBSTR, args);
		}else{
