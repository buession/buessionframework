 ...
 * @author Yong.Teng
 */
public final class JedisClusterSetOperations extends AbstractSetOperations<JedisCluster> {
	...
	public ScanResult<List<String>> sScan(final String key, final String cursor, final String pattern){
		...
		if(isPipeline()){
			...
			return execute(
					(cmd)->cmd.sscan(SafeEncoder.encode(key), SafeEncoder.encode(cursor), new JedisScanParams(pattern)),
					BINARY_TO_STRING_LIST_SCAN_RESULT_EXPOSE_CONVERTER, ProtocolCommand.SSCAN, args);
		}
	}
	...
	public ScanResult<List<String>> sScan(final String key, final String cursor, final long count){
		...
		if(isPipeline()){
			...
			return execute(
					(cmd)->cmd.sscan(SafeEncoder.encode(key), SafeEncoder.encode(cursor), new JedisScanParams(count)),
					BINARY_TO_STRING_LIST_SCAN_RESULT_EXPOSE_CONVERTER, ProtocolCommand.SSCAN, args);
		}
	}
	...
	public ScanResult<List<String>> sScan(final String key, final String cursor, final String pattern,
		...
		if(isPipeline()){
			...
			return execute((cmd)->cmd.sscan(SafeEncoder.encode(key), SafeEncoder.encode(cursor),
							new JedisScanParams(pattern, count)), BINARY_TO_STRING_LIST_SCAN_RESULT_EXPOSE_CONVERTER,
					ProtocolCommand.SSCAN, args);
		}
	}
