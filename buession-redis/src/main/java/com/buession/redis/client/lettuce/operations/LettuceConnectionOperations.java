...
public final class LettuceConnectionOperations extends AbstractConnectionOperations<LettuceStandaloneClient> {
	...
	public Status auth(final String password) {
		...
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, Command.AUTH, (cmd)->cmd.auth(password),
					okStatusConverter)
					.run(args);
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, Command.AUTH, (cmd)->cmd.auth(password),
					okStatusConverter)
					.run(args);
		}else{
			return new LettuceCommand<>(client, Command.AUTH, (cmd)->cmd.auth(password),
					okStatusConverter)
					.run(args);
		}
	}
	...
	public Long clientId() {
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, Command.CLIENT_ID, (cmd)->cmd.clientId(), (v)->v)
					.run();
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, Command.CLIENT_ID, (cmd)->cmd.clientId(), (v)->v)
					.run();
		}else{
			return new LettuceCommand<>(client, Command.CLIENT_ID, (cmd)->cmd.clientId(), (v)->v)
					.run();
		}
	}
	...
	public Status clientSetName(final byte[] name) {
		...
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, Command.CLIENT_SETNAME, (cmd)->cmd.clientSetname(name),
					okStatusConverter)
					.run(args);
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, Command.CLIENT_SETNAME,
					(cmd)->cmd.clientSetname(name), okStatusConverter)
					.run(args);
		}else{
			return new LettuceCommand<>(client, Command.CLIENT_SETNAME, (cmd)->cmd.clientSetname(name),
					okStatusConverter)
					.run(args);
		}
	}
	...
	public String clientGetName() {
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, Command.CLIENT_GETNAME, (cmd)->cmd.clientGetname(),
					SafeEncoder::encode)
					.run();
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, Command.CLIENT_GETNAME, (cmd)->cmd.clientGetname(),
					SafeEncoder::encode)
					.run();
		}else{
			return new LettuceCommand<>(client, Command.CLIENT_GETNAME, (cmd)->cmd.clientGetname(),
					SafeEncoder::encode)
					.run();
		}
	}
	...
	public List<Client> clientList() {
		...
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, Command.CLIENT_LIST, (cmd)->cmd.clientList(),
					clientListConverter)
					.run();
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, Command.CLIENT_LIST, (cmd)->cmd.clientList(),
					clientListConverter)
					.run();
		}else{
			return new LettuceCommand<>(client, Command.CLIENT_LIST, (cmd)->cmd.clientList(),
					clientListConverter)
					.run();
		}
	}
	...
	public Client clientInfo() {
		...
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, Command.CLIENT_INFO,
					(cmd)->cmd.clientInfo(), clientConverter)
					.run();
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, Command.CLIENT_INFO,
					(cmd)->cmd.clientInfo(), clientConverter)
					.run();
		}else{
			return new LettuceCommand<>(client, Command.CLIENT_INFO, (cmd)->cmd.clientInfo(),
					clientConverter)
					.run();
		}
	}
	...
	public Status clientSetInfo(final ClientAttributeOption clientAttributeOption, final String value) {
		...
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, Command.CLIENT_PAUSE,
					(cmd)->cmd.clientSetinfo(clientAttributeOption.name(), value), okStatusConverter)
					.run(args);
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, Command.CLIENT_PAUSE,
					(cmd)->cmd.clientSetinfo(clientAttributeOption.name(), value), okStatusConverter)
					.run(args);
		}else{
			return new LettuceCommand<>(client, Command.CLIENT_PAUSE,
	...
	public Status clientPause(final int timeout) {
		...
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, Command.CLIENT_PAUSE, (cmd)->cmd.clientPause(timeout),
					okStatusConverter)
					.run(args);
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, Command.CLIENT_PAUSE,
			...
			return new LettuceCommand<>(client, Command.CLIENT_PAUSE, (cmd)->cmd.clientPause(timeout),
					okStatusConverter)
					.run(args);
		}
	}
	...
	public Status clientKill(final String host, final int port) {
		...
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, Command.CLIENT_KILL, (cmd)->cmd.clientKill(addr),
					okStatusConverter)
					.run(args);
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, Command.CLIENT_KILL, (cmd)->cmd.clientKill(addr),
					okStatusConverter)
					.run(args);
		}else{
			return new LettuceCommand<>(client, Command.CLIENT_KILL, (cmd)->cmd.clientKill(addr),
					okStatusConverter)
					.run(args);
		}
	}
	...
	public Long clientKill(final ClientKillArgument... clientKillArguments) {
		...
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, Command.CLIENT_KILL, (cmd)->cmd.clientKill(killArgs),
					(v)->v)
					.run(args);
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, Command.CLIENT_KILL, (cmd)->cmd.clientKill(killArgs),
					(v)->v)
					.run(args);
		}else{
			return new LettuceCommand<>(client, Command.CLIENT_KILL, (cmd)->cmd.clientKill(killArgs),
					(v)->v)
					.run(args);
		}
	}
	...
	public Status clientNoEvict(final boolean on) {
		...
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, Command.CLIENT_NO_EVICT,
					(cmd)->cmd.clientNoEvict(on), okStatusConverter)
					.run(args);
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, Command.CLIENT_NO_EVICT,
					(cmd)->cmd.clientNoEvict(on), okStatusConverter)
					.run(args);
		}else{
			return new LettuceCommand<>(client, Command.CLIENT_NO_EVICT,
					(cmd)->cmd.clientNoEvict(on), okStatusConverter)
					.run(args);
		}
	}
	...
	private List<Client> clientList(final ClientListArgs clientListArgs, final CommandArguments args) {
		...
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, Command.CLIENT_LIST,
					(cmd)->cmd.clientList(clientListArgs), clientListConverter)
					.run(args);
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, Command.CLIENT_LIST,
					(cmd)->cmd.clientList(clientListArgs), clientListConverter)
					.run(args);
		}else{
			return new LettuceCommand<>(client, Command.CLIENT_LIST, (cmd)->cmd.clientList(clientListArgs),
					clientListConverter)
					.run(args);
		}
	}
	...
	private Status clientUnblock(final int clientId, final UnblockType unblockType, final CommandArguments args) {
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, Command.CLIENT_KILL,
					(cmd)->cmd.clientUnblock(clientId, unblockType), oneStatusConverter)
					.run(args);
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, Command.CLIENT_KILL,
					(cmd)->cmd.clientUnblock(clientId, unblockType), oneStatusConverter)
					.run(args);
		}else{
			return new LettuceCommand<>(client, Command.CLIENT_KILL,
					(cmd)->cmd.clientUnblock(clientId, unblockType), oneStatusConverter)
					.run(args);
		}
	}
