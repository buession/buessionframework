...
public final class LettuceConnectionOperations extends AbstractConnectionOperations<LettuceStandaloneClient> {
	...
	public Status auth(final String user, final String password) {
		...
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, ProtocolCommand.AUTH, (cmd)->cmd.auth(user, password),
					okStatusConverter)
					.run(args);
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, ProtocolCommand.AUTH, (cmd)->cmd.auth(user, password),
					okStatusConverter)
					.run(args);
		}else{
			return new LettuceCommand<>(client, ProtocolCommand.AUTH, (cmd)->cmd.auth(user, password),
					okStatusConverter)
					.run(args);
		}
	}
	...
	public Status auth(final String password) {
		...
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, ProtocolCommand.AUTH, (cmd)->cmd.auth(password),
					okStatusConverter)
					.run(args);
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, ProtocolCommand.AUTH, (cmd)->cmd.auth(password),
					okStatusConverter)
					.run(args);
		}else{
			return new LettuceCommand<>(client, ProtocolCommand.AUTH, (cmd)->cmd.auth(password),
					okStatusConverter)
					.run(args);
		}
	}
	...
	public Status ping() {
		...
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, ProtocolCommand.ECHO, (cmd)->cmd.ping(), pingResultConverter)
					.run();
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, ProtocolCommand.ECHO, (cmd)->cmd.ping(), pingResultConverter)
					.run();
		}else{
	...
	public Status quit() {
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, ProtocolCommand.QUIT, (cmd)->cmd.quit(), okStatusConverter)
					.run();
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, ProtocolCommand.QUIT, (cmd)->cmd.quit(), okStatusConverter)
					.run();
		}else{
	...
	public Status select(final int db) {
		...
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, ProtocolCommand.SELECT, (cmd)->cmd.select(db),
					okStatusConverter)
					.run(args);
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, ProtocolCommand.SELECT, (cmd)->cmd.select(db),
					okStatusConverter)
					.run(args);
		}else{
	...
	public Long clientId() {
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, ProtocolCommand.CLIENT_ID, (cmd)->cmd.clientId(), (v)->v)
					.run();
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, ProtocolCommand.CLIENT_ID, (cmd)->cmd.clientId(), (v)->v)
					.run();
		}else{
	...
	public String clientGetName() {
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, ProtocolCommand.CLIENT_GETNAME, (cmd)->cmd.clientGetname(),
					SafeEncoder::encode)
					.run();
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, ProtocolCommand.CLIENT_GETNAME, (cmd)->cmd.clientGetname(),
					SafeEncoder::encode)
					.run();
		}else{
	...
	public List<Client> clientList() {
		...
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, ProtocolCommand.CLIENT_LIST, (cmd)->cmd.clientList(),
					clientListConverter)
					.run();
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, ProtocolCommand.CLIENT_LIST, (cmd)->cmd.clientList(),
					clientListConverter)
					.run();
		}else{
	...
	public Client clientInfo() {
		...
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, ProtocolCommand.CLIENT_INFO,
					(cmd)->cmd.clientInfo(), clientConverter)
					.run();
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, ProtocolCommand.CLIENT_INFO,
					(cmd)->cmd.clientInfo(), clientConverter)
					.run();
		}else{
			return new LettuceCommand<>(client, ProtocolCommand.CLIENT_INFO, (cmd)->cmd.clientInfo(),
					clientConverter)
					.run();
		}
	}
	...
	public Status clientSetInfo(final ClientAttributeOption clientAttributeOption, final String value) {
		...
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, ProtocolCommand.CLIENT_PAUSE,
					(cmd)->cmd.clientSetinfo(clientAttributeOption.name(), value), okStatusConverter)
					.run(args);
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, ProtocolCommand.CLIENT_PAUSE,
					(cmd)->cmd.clientSetinfo(clientAttributeOption.name(), value), okStatusConverter)
					.run(args);
		}else{
			return new LettuceCommand<>(client, ProtocolCommand.CLIENT_PAUSE,
					(cmd)->cmd.clientSetinfo(clientAttributeOption.name(), value),
					okStatusConverter)
					.run(args);
		}
	}
	...
	public Status clientKill(final String host, final int port) {
		...
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, ProtocolCommand.CLIENT_KILL, (cmd)->cmd.clientKill(addr),
					okStatusConverter)
					.run(args);
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, ProtocolCommand.CLIENT_KILL, (cmd)->cmd.clientKill(addr),
					okStatusConverter)
					.run(args);
		}else{
	...
	public Long clientKill(final ClientKillArgument... clientKillArguments) {
		...
		if(isPipeline()){
			...
			return new LettuceTransactionCommand<>(client, ProtocolCommand.CLIENT_KILL, (cmd)->cmd.clientKill(killArgs),
					(v)->v)
					.run(args);
		}else{
			return new LettuceCommand<>(client, ProtocolCommand.CLIENT_KILL, (cmd)->cmd.clientKill(killArgs),
					(v)->v)
					.run(args);
		}
	}
	...
	public Status clientNoEvict(final boolean on) {
		...
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, ProtocolCommand.CLIENT_NO_EVICT,
					(cmd)->cmd.clientNoEvict(on), okStatusConverter)
					.run(args);
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, ProtocolCommand.CLIENT_NO_EVICT,
					(cmd)->cmd.clientNoEvict(on), okStatusConverter)
					.run(args);
		}else{
			return new LettuceCommand<>(client, ProtocolCommand.CLIENT_NO_EVICT,
					(cmd)->cmd.clientNoEvict(on), okStatusConverter)
					.run(args);
		}
	}
	...
	private <V> V echo(final byte[] str, final Converter<byte[], V> converter, final CommandArguments args) {
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, ProtocolCommand.ECHO, (cmd)->cmd.echo(str), converter)
					.run(args);
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, ProtocolCommand.ECHO, (cmd)->cmd.echo(str), converter)
					.run(args);
		}else{
	...
	private List<Client> clientList(final ClientListArgs clientListArgs, final CommandArguments args) {
		...
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, ProtocolCommand.CLIENT_LIST,
					(cmd)->cmd.clientList(clientListArgs), clientListConverter)
					.run(args);
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, ProtocolCommand.CLIENT_LIST,
					(cmd)->cmd.clientList(clientListArgs), clientListConverter)
					.run(args);
		}else{
			return new LettuceCommand<>(client, ProtocolCommand.CLIENT_LIST, (cmd)->cmd.clientList(clientListArgs),
					clientListConverter)
					.run(args);
		}
	}
	...
	private Status clientUnblock(final int clientId, final UnblockType unblockType, final CommandArguments args) {
		if(isPipeline()){
			return new LettucePipelineCommand<>(client, ProtocolCommand.CLIENT_KILL,
					(cmd)->cmd.clientUnblock(clientId, unblockType), oneStatusConverter)
					.run(args);
		}else if(isTransaction()){
			return new LettuceTransactionCommand<>(client, ProtocolCommand.CLIENT_KILL,
					(cmd)->cmd.clientUnblock(clientId, unblockType), oneStatusConverter)
					.run(args);
		}else{
