 ...
 * @since 2.0.0
 */
public final class JedisClusterHashOperations extends AbstractHashOperations<JedisClusterConnection> {
	...
	public long hDel(final String key, final String... fields){
		...
		final JedisClusterCommand<Long> command = JedisClusterCommand.<Long>create(ProtocolCommand.HDEL)
	...
	public boolean hExists(final String key, final String field){
		...
		final JedisClusterCommand<Boolean> command = JedisClusterCommand.<Boolean>create(ProtocolCommand.HEXISTS)
	...
	public String hGet(final String key, final String field){
		final CommandArguments args = CommandArguments.create("key", key).put("field", field);
		final JedisClusterCommand<String> command = JedisClusterCommand.<String>create(ProtocolCommand.HGET)
	...
	public byte[] hGet(final byte[] key, final byte[] field){
		final CommandArguments args = CommandArguments.create("key", key).put("field", field);
		final JedisClusterCommand<byte[]> command = JedisClusterCommand.<byte[]>create(ProtocolCommand.HGET)
				.general((cmd)->cmd.hget(key, field)).pipeline((cmd)->cmd.hget(key, field))
				.transaction((cmd)->cmd.hget(key, field));
		return execute(command, args);
	}
	...
	public Map<String, String> hGetAll(final String key){
		...
		final JedisClusterCommand<Map<String, String>> command = JedisClusterCommand.<Map<String, String>>create(
	...
	public Map<byte[], byte[]> hGetAll(final byte[] key){
		final CommandArguments args = CommandArguments.create("key", key);
		final JedisClusterCommand<Map<byte[], byte[]>> command = JedisClusterCommand.<Map<byte[], byte[]>>create(
						ProtocolCommand.HGETALL)
				.general((cmd)->cmd.hgetAll(key)).pipeline((cmd)->cmd.hgetAll(key))
				.transaction((cmd)->cmd.hgetAll(key));
		return execute(command, args);
	...
	public ScanResult<Map<String, String>> hScan(final String key, final String cursor){
		...
		final JedisClusterCommand<ScanResult<Map<String, String>>> command = JedisClusterCommand.<ScanResult<Map<String, String>>>create(
	...
	public ScanResult<Map<byte[], byte[]>> hScan(final byte[] key, final byte[] cursor){
		final CommandArguments args = CommandArguments.create("key", key).put("cursor", cursor);
		final JedisClusterCommand<ScanResult<Map<byte[], byte[]>>> command = JedisClusterCommand.<ScanResult<Map<byte[], byte[]>>>create(
						ProtocolCommand.HSCAN)
