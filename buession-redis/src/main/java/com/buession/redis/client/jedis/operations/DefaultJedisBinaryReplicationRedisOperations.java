 ...
 * @author Yong.Teng
 */
public class DefaultJedisBinaryReplicationRedisOperations<C extends BinaryJedisCommands> extends AbstractJedisBinaryRedisOperations implements JedisBinaryReplicationRedisOperations {

	public DefaultJedisBinaryReplicationRedisOperations(final JedisRedisClient client){
		super(client);
	}
			...
			public Status execute(C cmd){
				if(isTransaction()){
					...
					if(cmd instanceof Jedis){
						return statusForOK(((Jedis) cmd).slaveof(SafeEncoder.encode(host), port));
					}else{
