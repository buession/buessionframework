 ...
 * @author Yong.Teng
 */
public class ShardedJedisBinaryBitMapRedisOperations extends AbstractJedisBinaryBitMapRedisOperations<ShardedJedis> {

	public ShardedJedisBinaryBitMapRedisOperations(final JedisRedisClient client){
		super(client);
	}
	...
	public Long bitPos(final byte[] key, final boolean value){
		...
		if(isTransaction()){
			...
			return execute((ShardedJedis cmd)->cmd.bitpos(SafeEncoder.encode(key), value), ProtocolCommand.BITPOS,
					args);
		}
	}
	...
	public Long bitPos(final byte[] key, final boolean value, final int start, final int end){
		...
		if(isTransaction()){
			return execute((ShardedJedis cmd)->getTransaction().bitpos(key, value, bitPosParams).get(),
					ProtocolCommand.BITPOS, args);
		}else{
			return execute((ShardedJedis cmd)->cmd.bitpos(SafeEncoder.encode(key), value, bitPosParams),
					ProtocolCommand.BITPOS, args);
		}
	}
