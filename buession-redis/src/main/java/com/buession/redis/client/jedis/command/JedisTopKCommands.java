...
public final class JedisTopKCommands extends AbstractJedisRedisCommands implements TopKCommands {
	...
	public List<String> topKAdd(final String key, final String... items) {
		...
		return executeCommand(RedisCommand.TOPK_ADD, args, (cmd)->cmd.topkAdd(key, items),
				(cmd)->cmd.topkAdd(key, items), (cmd)->cmd.topkAdd(key, items));
	}
	...
	public List<byte[]> topKAdd(final byte[] key, final byte[]... items) {
		...
		return executeCommand(RedisCommand.TOPK_ADD, args,
				(cmd)->cmd.topkAdd(SafeEncoder.encode(key), SafeEncoder.encode(items)),
				(cmd)->cmd.topkAdd(SafeEncoder.encode(key), SafeEncoder.encode(items)),
				(cmd)->cmd.topkAdd(SafeEncoder.encode(key), SafeEncoder.encode(items)),
	...
	public List<String> topKIncrBy(final String key, final KeyValue<String, Long>... items) {
		...
		return executeCommand(RedisCommand.TOPK_INCRBY, args,
				(cmd)->cmd.topkIncrBy(key, arrayKeyValueMapConverter.convert(items)),
				(cmd)->cmd.topkIncrBy(key, arrayKeyValueMapConverter.convert(items)),
				(cmd)->cmd.topkIncrBy(key, arrayKeyValueMapConverter.convert(items)));
	}
	...
	public List<byte[]> topKIncrBy(final byte[] key, final KeyValue<byte[], Long>... items) {
		...
		return executeCommand(RedisCommand.TOPK_INCRBY, args,
				(cmd)->cmd.topkIncrBy(SafeEncoder.encode(key), arrayKeyValueMapConverter.convert(items)),
				(cmd)->cmd.topkIncrBy(SafeEncoder.encode(key), arrayKeyValueMapConverter.convert(items)),
				(cmd)->cmd.topkIncrBy(SafeEncoder.encode(key), arrayKeyValueMapConverter.convert(items)),
				Converters.stringListBinaryListConverter());
	}
	...
	public TopKInfo topKInfo(final String key) {
		...
		return executeCommand(RedisCommand.TOPK_INFO, args, (cmd)->cmd.topkInfo(key),
				(cmd)->cmd.topkInfo(key), (cmd)->cmd.topkInfo(key), new TopKInfoConverter());
	}
	...
	public List<String> topKList(final String key) {
		final CommandArguments args = CommandArguments.create(key);
		return executeCommand(RedisCommand.TOPK_LIST, args, (cmd)->cmd.topkList(key),
				(cmd)->cmd.topkList(key), (cmd)->cmd.topkList(key));
	}
	...
	public List<Boolean> topKQuery(final String key, final String... items) {
		final CommandArguments args = CommandArguments.create(key).add(items);
		return executeCommand(RedisCommand.TOPK_QUERY, args, (cmd)->cmd.topkQuery(key, items),
				(cmd)->cmd.topkQuery(key, items), (cmd)->cmd.topkQuery(key, items));
	}
