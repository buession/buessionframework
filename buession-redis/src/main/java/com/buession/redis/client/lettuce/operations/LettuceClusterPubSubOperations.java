...
public final class LettuceClusterPubSubOperations extends AbstractPubSubOperations<LettuceClusterClient> {
	...
	public Long publish(final byte[] channel, final byte[] message) {
		...
		if(isPipeline()){
			...
			return new LettuceClusterCommand<>(client, ProtocolCommand.PUBLISH, (cmd)->cmd.publish(channel, message),
					(v)->v)
					.run(args);
		}
	}
	...
	public Long pubsubNumPat() {
		if(isPipeline()){
			return new LettuceClusterPipelineCommand<>(client, ProtocolCommand.PUBSUB_NUMPAT, (cmd)->cmd.pubsubNumpat(),
					(v)->v)
					.run();
		}else if(isTransaction()){
	...
	public Object pUnSubscribe() {
		if(isPipeline()){
			return new LettuceClusterPipelineCommand<>(client, ProtocolCommand.PUNSUBSCRIBE)
					.run();
		}else if(isTransaction()){
			return new LettuceClusterTransactionCommand<>(client, ProtocolCommand.PUNSUBSCRIBE)
					.run();
		}else{
			return new LettuceClusterCommand<>(client, ProtocolCommand.PUNSUBSCRIBE)
					.run();
		}
	}
	...
	public Object unSubscribe() {
		if(isPipeline()){
			return new LettuceClusterPipelineCommand<>(client, ProtocolCommand.UNSUBSCRIBE)
					.run();
		}else if(isTransaction()){
			return new LettuceClusterTransactionCommand<>(client, ProtocolCommand.UNSUBSCRIBE)
					.run();
		}else{
			return new LettuceClusterCommand<>(client, ProtocolCommand.UNSUBSCRIBE)
					.run();
		}
	}
	...
	private void pSubscribe(final CommandArguments args) {
		if(isPipeline()){
			new LettuceClusterPipelineCommand<>(client, ProtocolCommand.PSUBSCRIBE)
					.run(args);
		}else if(isTransaction()){
			new LettuceClusterTransactionCommand<>(client, ProtocolCommand.PSUBSCRIBE)
					.run(args);
		}else{
			new LettuceClusterCommand<>(client, ProtocolCommand.PSUBSCRIBE)
					.run(args);
		}
	}
	...
	private <K> Map<K, Long> pubsubNumSub(final byte[][] channels,
										  ...
										  final CommandArguments args) {
		if(isPipeline()){
			return new LettuceClusterPipelineCommand<>(client, ProtocolCommand.PUBSUB_NUMSUB,
					(cmd)->cmd.pubsubNumsub(channels), converter)
					.run(args);
		}else if(isTransaction()){
			return new LettuceClusterTransactionCommand<>(client, ProtocolCommand.PUBSUB_NUMSUB,
					(cmd)->cmd.pubsubNumsub(channels), converter)
					.run(args);
		}else{
			return new LettuceClusterCommand<>(client, ProtocolCommand.PUBSUB_NUMSUB, (cmd)->cmd.pubsubNumsub(channels),
					converter)
					.run(args);
		}
	}
	...
	private void subscribe(final CommandArguments args) {
		if(isPipeline()){
			new LettuceClusterPipelineCommand<>(client, ProtocolCommand.SUBSCRIBE)
					.run(args);
		}else if(isTransaction()){
			new LettuceClusterTransactionCommand<>(client, ProtocolCommand.SUBSCRIBE)
					.run(args);
		}else{
			new LettuceClusterCommand<>(client, ProtocolCommand.SUBSCRIBE)
					.run(args);
		}
	}
