 ...
 * @author Yong.Teng
 */
public class DefaultJedisListRedisOperations<C extends JedisCommands> extends AbstractJedisRedisOperations implements JedisListRedisOperations {

	public DefaultJedisListRedisOperations(final JedisRedisClient client){
		super(client);
	}
				...
				if(isTransaction()){
					...
					if(cmd instanceof Jedis){
						...
						if(Validate.isEmpty(keys)){
							...
							for(String key : keys){
								ret = cmd.brpop(timeout, key);

								if(Validate.isEmpty(ret) == false){
									result.addAll(ret);
								}
							}
