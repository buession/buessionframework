...
public final class JedisSentinelClusterOperations extends AbstractClusterOperations<JedisSentinelClient> {
	...
	public String clusterMyId(){
		final JedisSentinelCommand<String> command = new JedisSentinelCommand<String>(client,
				ProtocolCommand.CLUSTER_MY_ID).general((cmd)->cmd.clusterMyId());
	...
	public Status clusterAddSlots(final int... slots){
		...
		final JedisSentinelCommand<Status> command = new JedisSentinelCommand<Status>(client,
				ProtocolCommand.CLUSTER_ADDSLOTS).general((cmd)->cmd.clusterAddSlots(slots),
	...
	public List<ClusterSlot> clusterSlots(){
		final JedisSentinelCommand<List<ClusterSlot>> command = new JedisSentinelCommand<List<ClusterSlot>>(client,
	...
	public Status clusterDelSlots(final int... slots){
		final CommandArguments args = CommandArguments.create("slots", slots);
		final JedisSentinelCommand<Status> command = new JedisSentinelCommand<Status>(client,
				ProtocolCommand.CLUSTER_DELSLOTS).general((cmd)->cmd.clusterDelSlots(slots),
				OkStatusConverter.INSTANCE);
		return execute(command, args);
	}
	...
	public Status clusterFlushSlots(){
		final JedisSentinelCommand<Status> command = new JedisSentinelCommand<Status>(client,
				ProtocolCommand.CLUSTER_FLUSHSLOTS).general((cmd)->cmd.clusterFlushSlots(), OkStatusConverter.INSTANCE);
		return execute(command);
	}
	...
	public Status clusterFailover(final ClusterFailoverOption clusterFailoverOption){
		...
		final JedisSentinelCommand<Status> command = new JedisSentinelCommand<Status>(client,
				ProtocolCommand.CLUSTER_FAILOVER).general(
				(cmd)->cmd.clusterFailover(ClusterFailoverOptionConverter.INSTANCE.convert(clusterFailoverOption)),
				OkStatusConverter.INSTANCE);
		return execute(command, args);
	}
	...
	public Status clusterForget(final String nodeId){
		final CommandArguments args = CommandArguments.create("nodeId", nodeId);
		final JedisSentinelCommand<Status> command = new JedisSentinelCommand<Status>(client,
				ProtocolCommand.CLUSTER_FORGET).general((cmd)->cmd.clusterForget(nodeId), OkStatusConverter.INSTANCE);
		return execute(command, args);
	}
	...
	public ClusterInfo clusterInfo(){
		final JedisSentinelCommand<ClusterInfo> command = new JedisSentinelCommand<ClusterInfo>(client,
				ProtocolCommand.CLUSTER_INFO).general((cmd)->cmd.clusterInfo(), ClusterInfoConverter.INSTANCE);
		return execute(command);
	}
	...
	public Status clusterMeet(final String ip, final int port){
		...
		final JedisSentinelCommand<Status> command = new JedisSentinelCommand<Status>(client,
				ProtocolCommand.CLUSTER_MEET).general((cmd)->cmd.clusterMeet(ip, port), OkStatusConverter.INSTANCE);
		return execute(command, args);
	}

	@Override
	public List<RedisClusterServer> clusterNodes(){
		final JedisSentinelCommand<List<RedisClusterServer>> command = new JedisSentinelCommand<List<RedisClusterServer>>(
	...
	public List<RedisClusterServer> clusterSlaves(final String nodeId){
		final CommandArguments args = CommandArguments.create("nodeId", nodeId);
		final JedisSentinelCommand<List<RedisClusterServer>> command = new JedisSentinelCommand<List<RedisClusterServer>>(
				client, ProtocolCommand.CLUSTER_SLAVES).general((cmd)->cmd.clusterSlaves(nodeId),
				ClusterSlaveConverter.LIST_CONVERTER);
		return execute(command, args);
	}
	...
	public List<RedisClusterServer> clusterReplicas(final String nodeId){
		final CommandArguments args = CommandArguments.create("nodeId", nodeId);
		final JedisSentinelCommand<List<RedisClusterServer>> command = new JedisSentinelCommand<List<RedisClusterServer>>(
				client, ProtocolCommand.CLUSTER_REPLICAS).general((cmd)->cmd.clusterReplicas(nodeId),
	...
	public Status clusterReplicate(final String nodeId){
		final CommandArguments args = CommandArguments.create("nodeId", nodeId);
		final JedisSentinelCommand<Status> command = new JedisSentinelCommand<Status>(client,
				ProtocolCommand.CLUSTER_REPLICATE).general((cmd)->cmd.clusterReplicate(nodeId),
				OkStatusConverter.INSTANCE);
		return execute(command, args);
	}
	...
	public Status clusterReset(final ClusterResetOption clusterResetOption){
		...
		final JedisSentinelCommand<Status> command = new JedisSentinelCommand<Status>(client,
				ProtocolCommand.CLUSTER_RESET).general((cmd)->cmd.clusterReset(
	...
	public Status clusterSaveConfig(){
		final JedisSentinelCommand<Status> command = new JedisSentinelCommand<Status>(client,
				ProtocolCommand.CLUSTER_SAVECONFIG).general((cmd)->cmd.clusterSaveConfig(), OkStatusConverter.INSTANCE);
		return execute(command);
	}
	...
	public BumpEpoch clusterBumpEpoch(){
		final JedisSentinelCommand<BumpEpoch> command = new JedisSentinelCommand<BumpEpoch>(client,
				ProtocolCommand.CLUSTER_BUMPEPOCH).general((cmd)->cmd.clusterBumpEpoch(), BumpEpochConverter.INSTANCE);
		return execute(command);
	}
	...
	public Status asking(){
		final JedisSentinelCommand<Status> command = new JedisSentinelCommand<Status>(client, ProtocolCommand.ASKING)
