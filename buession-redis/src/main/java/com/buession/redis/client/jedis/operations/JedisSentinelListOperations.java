 ...
 * @author Yong.Teng
 */
public final class JedisSentinelListOperations extends AbstractListOperations<Jedis> {
	...
	public Long lPos(final String key, final String element){
		...
		if(isPipeline()){
			return pipelineExecute((cmd)->newJedisResult(getPipeline().lpos(key, element)), ProtocolCommand.LPOS, args);
		}else if(isTransaction()){
			return transactionExecute((cmd)->newJedisResult(getTransaction().lpos(key, element)), ProtocolCommand.LPOS,
					args);
		}else{
	...
	public Long lPos(final String key, final String element, final LPosArgument lPosArgument){
		...
		if(isPipeline()){
			return pipelineExecute((cmd)->newJedisResult(getPipeline().lpos(key, element, lPosParams)),
					ProtocolCommand.LPOP, args);
		}else if(isTransaction()){
			return transactionExecute((cmd)->newJedisResult(getTransaction().lpos(key, element, lPosParams)),
					ProtocolCommand.LPOP, args);
		}else{
			return execute((cmd)->cmd.lpos(key, element, lPosParams), ProtocolCommand.LPOP, args);
		}
	}
