...
public class ShardedJedisKeyOperations extends AbstractKeyOperations<ShardedJedis, ShardedJedisPipeline> {
	...
	public Status migrate(final String key, final String host, final int port, final int db, final int timeout){
		...
		if(isPipeline()){
			...
			return execute((cmd)->JedisClientUtils.getShard(cmd, key).migrate(host, port, key, db, timeout),
					converter);
		}
	}
	...
	public Status migrate(final String key, final String host, final int port, final int db, final int timeout,
		...
		if(isPipeline()){
			...
			return execute((cmd)->JedisClientUtils.getShard(cmd, key).migrate(host, port, db, timeout, migrateParams,
					key), converter);
		}
	}
	...
	public List<byte[]> sort(final byte[] key, final SortArgument sortArgument){
		final SortingParams sortingParams = new SortArgumentJedisConverter().convert(sortArgument);
