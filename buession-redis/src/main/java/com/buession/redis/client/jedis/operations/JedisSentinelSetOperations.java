 ...
 * @since 2.0.0
 */
public final class JedisSentinelSetOperations extends AbstractSetOperations<JedisSentinelConnection> {
	...
	public long sAdd(final String key, final String... members){
		...
		final JedisSentinelCommand<Long> command = JedisSentinelCommand.<Long>create(ProtocolCommand.SADD)
	...
	public long sCard(final String key){
		...
		final JedisSentinelCommand<Long> command = JedisSentinelCommand.<Long>create(ProtocolCommand.SCARD)
	...
	public Set<String> sDiff(final String... keys){
		...
		final JedisSentinelCommand<Set<String>> command = JedisSentinelCommand.<Set<String>>create(
						ProtocolCommand.SDIFF).general((cmd)->cmd.sdiff(keys)).pipeline((cmd)->cmd.sdiff(keys))
	...
	public long sDiffStore(final String destKey, final String... keys){
		...
		final JedisSentinelCommand<Long> command = JedisSentinelCommand.<Long>create(ProtocolCommand.SDIFFSTORE)
	...
	public Set<String> sInter(final String... keys){
		final CommandArguments args = CommandArguments.create("keys", keys);
		final JedisSentinelCommand<Set<String>> command = JedisSentinelCommand.<Set<String>>create(
						ProtocolCommand.SINTER).general((cmd)->cmd.sinter(keys)).pipeline((cmd)->cmd.sinter(keys))
	...
	public long sInterStore(final String destKey, final String... keys){
		final CommandArguments args = CommandArguments.create("destKey", destKey).put("keys", keys);
		final JedisSentinelCommand<Long> command = JedisSentinelCommand.<Long>create(ProtocolCommand.SINTERSTORE)
	...
	public boolean sIsMember(final String key, final String member){
		...
		final JedisSentinelCommand<Boolean> command = JedisSentinelCommand.<Boolean>create(ProtocolCommand.SISMEMBER)
	...
	public Status sMove(final String key, final String destKey, final String member){
		...
		final JedisSentinelCommand<Status> command = JedisSentinelCommand.<Status>create(ProtocolCommand.SMOVE)
	...
	public String sPop(final String key){
		final CommandArguments args = CommandArguments.create("key", key);
		final JedisSentinelCommand<String> command = JedisSentinelCommand.<String>create(ProtocolCommand.SPOP)
	...
	public byte[] sPop(final byte[] key){
		final CommandArguments args = CommandArguments.create("key", key);
		final JedisSentinelCommand<byte[]> command = JedisSentinelCommand.<byte[]>create(ProtocolCommand.SPOP)
				.general((cmd)->cmd.spop(key)).pipeline((cmd)->cmd.spop(key)).transaction((cmd)->cmd.spop(key));
		return execute(command, args);
	}
	...
	public Set<String> sPop(final String key, final long count){
		...
		final JedisSentinelCommand<Set<String>> command = JedisSentinelCommand.<Set<String>>create(ProtocolCommand.SPOP)
	...
	public Set<byte[]> sPop(final byte[] key, final long count){
		final CommandArguments args = CommandArguments.create("key", key).put("count", count);
		final JedisSentinelCommand<Set<byte[]>> command = JedisSentinelCommand.<Set<byte[]>>create(ProtocolCommand.SPOP)
				.general((cmd)->cmd.spop(key, count)).pipeline((cmd)->cmd.spop(key, count))
				.transaction((cmd)->cmd.spop(key, count));
		return execute(command, args);
	}
	...
	public String sRandMember(final String key){
		final CommandArguments args = CommandArguments.create("key", key);
		final JedisSentinelCommand<String> command = JedisSentinelCommand.<String>create(ProtocolCommand.SRANDMEMBER)
	...
	public byte[] sRandMember(final byte[] key){
		final CommandArguments args = CommandArguments.create("key", key);
		final JedisSentinelCommand<byte[]> command = JedisSentinelCommand.<byte[]>create(ProtocolCommand.SRANDMEMBER)
				.general((cmd)->cmd.srandmember(key)).pipeline((cmd)->cmd.srandmember(key));
		return execute(command, args);
	}
	...
	public long sRem(final String key, final String... members){
		final CommandArguments args = CommandArguments.create("key", key).put("members", members);
		final JedisSentinelCommand<Long> command = JedisSentinelCommand.<Long>create(ProtocolCommand.SREM)
	...
	public Set<String> sUnion(final String... keys){
		final CommandArguments args = CommandArguments.create("keys", keys);
		final JedisSentinelCommand<Set<String>> command = JedisSentinelCommand.<Set<String>>create(
						ProtocolCommand.SUNION).general((cmd)->cmd.sunion(keys)).pipeline((cmd)->cmd.sunion(keys))
	...
	public long sUnionStore(final String destKey, final String... keys){
		final CommandArguments args = CommandArguments.create("destKey", destKey).put("keys", keys);
		final JedisSentinelCommand<Long> command = JedisSentinelCommand.<Long>create(ProtocolCommand.SUNIONSTORE)
