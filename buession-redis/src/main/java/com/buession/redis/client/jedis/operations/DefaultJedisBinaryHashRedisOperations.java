 ...
 * @author Yong.Teng
 */
public class DefaultJedisBinaryHashRedisOperations<C extends BinaryJedisCommands> extends AbstractJedisBinaryRedisOperations implements JedisBinaryHashRedisOperations {

	public DefaultJedisBinaryHashRedisOperations(final JedisRedisClient client){
		super(client);
	}
	...
	public ScanResult<Map<byte[], byte[]>> hScan(final byte[] key, final byte[] cursor){
		...
		return execute(new Executor<C, ScanResult<Map<byte[], byte[]>>>() {
			...
			@Override
			public ScanResult<Map<byte[], byte[]>> execute(C cmd){
				if(isTransaction()){
