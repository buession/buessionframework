...
public class ShardedJedisClient extends AbstractJedisRedisClient<ShardedJedis> implements ShardedRedisClient {
    ...
    public Status restore(final byte[] key, final byte[] serializedValue, final int ttl){
        return execute(ProtocolCommand.RESTORE, (ShardedJedis client)->returnForOK(client.restore(key, ttl,
    ...
    public Status migrate(final String key, final String host, final int port, final int db, final int timeout){
        return execute(ProtocolCommand.MIGRATE, (ShardedJedis client)->returnForOK(getShard(client, key).migrate
                (host, port, key, db, timeout)), OperationsCommandArguments.getInstance().put("key", key).put("host",
    ...
    MigrateOperation migrateOperation){
        return execute(ProtocolCommand.MIGRATE, (ShardedJedis client)->returnForOK(getShard(client, key).migrate
                (host, port, db, timeout, (new MigrateOperationConvert()).convert(migrateOperation), key)),
                OperationsCommandArguments.getInstance().put("key", key).put("host", host).put("port", port).put
                        ("db", db).put("timeout", timeout).put("migrate", migrateOperation));
    }
    ...
    public Status set(final byte[] key, final byte[] value){
        return execute(ProtocolCommand.SET, (ShardedJedis client)->returnForOK(client.set(key, value)),
    ...
    public Status set(final byte[] key, final byte[] value, final SetArgument setArgument){
        return execute(ProtocolCommand.SET, (ShardedJedis client)->returnForOK(client.set(key, value, (new
    ...
    public Status setEx(final byte[] key, final byte[] value, final int lifetime){
        return execute(ProtocolCommand.SETEX, (ShardedJedis client)->returnForOK(client.setex(key, lifetime, value)),
                OperationsCommandArguments.getInstance().put("key", key).put("value", value).put("lifetime", lifetime));
    }

    @Override
    public Status pSetEx(final byte[] key, final byte[] value, final int lifetime){
        return execute(ProtocolCommand.PSETEX, (ShardedJedis client)->returnForOK(client.psetex(key, lifetime, value)
        ), OperationsCommandArguments.getInstance().put("key", key).put("value", value).put("lifetime", lifetime));
    }
    ...
    public Status hMSet(final byte[] key, final Map<byte[], byte[]> data){
        return execute(ProtocolCommand.HSETNX, (ShardedJedis client)->returnForOK(client.hmset(key, data)),
    ...
    public ScanResult<Map<byte[], byte[]>> hScan(final byte[] key, final byte[] cursor, final int count){
        return execute(ProtocolCommand.HSCAN, (ShardedJedis client)->(new ScanResultConvert
                ...
                (count))), OperationsCommandArguments.getInstance().put("key", key).put("cursor", cursor).put
                ("count", count));
    }
    ...
    public Status lSet(final byte[] key, final long index, final byte[] value){
        return execute(ProtocolCommand.LSET, (ShardedJedis client)->returnForOK(client.lset(key, index, value)),
    ...
    public Status lTrim(final byte[] key, final long start, final long end){
        return execute(ProtocolCommand.LTRIM, (ShardedJedis client)->returnForOK(client.ltrim(key, start, end)),
                OperationsCommandArguments.getInstance().put("key", key).put("start", start).put("end", end));
    }
    ...
    public Long zRemRangeByRank(final byte[] key, final long start, final long end){
        return execute(ProtocolCommand.ZREMRANGEBYRANK, (ShardedJedis client)->client.zremrangeByRank(key, start,
                end), OperationsCommandArguments.getInstance().put("key", key).put("start", start).put("end", end));
    }

    @Override
    public Long zRemRangeByScore(final byte[] key, final double min, final double max){
        return execute(ProtocolCommand.ZREMRANGEBYSCORE, (ShardedJedis client)->client.zremrangeByScore(key, min,
                max), OperationsCommandArguments.getInstance().put("key", key).put("min", min).put("max", max));
    }
    ...
    public Set<byte[]> zRevRangeByScore(final byte[] key, final double min, final double max){
        return execute(ProtocolCommand.ZREVRANGEBYSCORE, (ShardedJedis client)->client.zrevrangeByScore(key, max,
                min), OperationsCommandArguments.getInstance().put("key", key).put("min", min).put("max", max));
    }
    ...
    int count){
        return execute(ProtocolCommand.ZREVRANGEBYSCORE, (ShardedJedis client)->client.zrevrangeByScore(key, max,
                min, offset, count), OperationsCommandArguments.getInstance().put("key", key).put("min", min).put
    ...
    public ScanResult<List<Tuple>> zScan(final byte[] key, final byte[] cursor){
        return execute(ProtocolCommand.ZSCAN, (ShardedJedis client)->(new ScanResultConvert
                .ListTupleScanResultConvert()).deconvert(client.zscan(key, cursor)), OperationsCommandArguments
                .getInstance().put("key", key).put("cursor", cursor));
    }
    ...
    public ScanResult<List<Tuple>> zScan(final byte[] key, final byte[] cursor, final byte[] pattern){
        return execute(ProtocolCommand.ZSCAN, (ShardedJedis client)->(new ScanResultConvert
                .ListTupleScanResultConvert()).deconvert(client.zscan(key, cursor, (new ScanParams()).match(pattern))
    ...
    public ScanResult<List<Tuple>> zScan(final byte[] key, final byte[] cursor, final int count){
        return execute(ProtocolCommand.ZSCAN, (ShardedJedis client)->(new ScanResultConvert
                .ListTupleScanResultConvert()).deconvert(client.zscan(key, cursor, (new ScanParams()).count(count))),
                OperationsCommandArguments.getInstance().put("key", key).put("cursor", cursor).put("count", count));
    }
    ...
    public ScanResult<List<Tuple>> zScan(final byte[] key, final byte[] cursor, final byte[] pattern, final int count){
        return execute(ProtocolCommand.ZSCAN, (ShardedJedis client)->(new ScanResultConvert
                .ListTupleScanResultConvert()).deconvert(client.zscan(key, cursor, (new ScanParams()).match(pattern)
                .count(count))), OperationsCommandArguments.getInstance().put("key", key).put("cursor", cursor).put
    ...
    public Long geoAdd(final byte[] key, final byte[] member, final double longitude, final double latitude){
        return execute(ProtocolCommand.GEOADD, (ShardedJedis client)->client.geoadd(key, longitude, latitude, member)
                , OperationsCommandArguments.getInstance().put("key", key).put("longitude", longitude).put
                        ("latitude", latitude).put("member", member));
    }
    ...
    public List<Geo> geoPos(final byte[] key, final byte[]... members){
        return execute(ProtocolCommand.GEOPOS, (ShardedJedis client)->(new GeoConvert.ListMapConvert()).deconvert
                (client.geopos(key, members)), OperationsCommandArguments.getInstance().put("key", key).put
                ("members", members));
    }
