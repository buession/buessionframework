 ...
 * @since 2.0.0
 */
public final class JedisSentinelHashOperations extends AbstractHashOperations<JedisSentinelConnection> {
	...
	public long hDel(final String key, final String... fields){
		...
		final JedisSentinelCommand<Long> command = JedisSentinelCommand.<Long>create(ProtocolCommand.HDEL)
	...
	public boolean hExists(final String key, final String field){
		...
		final JedisSentinelCommand<Boolean> command = JedisSentinelCommand.<Boolean>create(ProtocolCommand.HEXISTS)
	...
	public String hGet(final String key, final String field){
		final CommandArguments args = CommandArguments.create("key", key).put("field", field);
		final JedisSentinelCommand<String> command = JedisSentinelCommand.<String>create(ProtocolCommand.HGET)
	...
	public byte[] hGet(final byte[] key, final byte[] field){
		final CommandArguments args = CommandArguments.create("key", key).put("field", field);
		final JedisSentinelCommand<byte[]> command = JedisSentinelCommand.<byte[]>create(ProtocolCommand.HGET)
				.general((cmd)->cmd.hget(key, field)).pipeline((cmd)->cmd.hget(key, field))
				.transaction((cmd)->cmd.hget(key, field));
		return execute(command, args);
	}
	...
	public Map<String, String> hGetAll(final String key){
		...
		final JedisSentinelCommand<Map<String, String>> command = JedisSentinelCommand.<Map<String, String>>create(
	...
	public Map<byte[], byte[]> hGetAll(final byte[] key){
		final CommandArguments args = CommandArguments.create("key", key);
		final JedisSentinelCommand<Map<byte[], byte[]>> command = JedisSentinelCommand.<Map<byte[], byte[]>>create(
						ProtocolCommand.HGETALL)
				.general((cmd)->cmd.hgetAll(key)).pipeline((cmd)->cmd.hgetAll(key))
				.transaction((cmd)->cmd.hgetAll(key));
		return execute(command, args);
	...
	public String hRandField(final String key){
		final CommandArguments args = CommandArguments.create("key", key);
		final JedisSentinelCommand<String> command = JedisSentinelCommand.<String>create(
	...
	public byte[] hRandField(final byte[] key){
		final CommandArguments args = CommandArguments.create("key", key);
		final JedisSentinelCommand<byte[]> command = JedisSentinelCommand.<byte[]>create(
						ProtocolCommand.HRANDFIELD).general((cmd)->cmd.hrandfield(key))
				.pipeline((cmd)->cmd.hrandfield(key)).transaction((cmd)->cmd.hrandfield(key));
		return execute(command, args);
	}
