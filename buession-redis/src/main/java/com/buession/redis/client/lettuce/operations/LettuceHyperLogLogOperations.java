...
public final class LettuceHyperLogLogOperations extends AbstractHyperLogLogOperations<LettuceStandaloneClient> {
	...
	public Status pfAdd(final byte[] key, final byte[]... elements) {
		...
		if(isPipeline()){
			...
			return new LettuceCommand<>(client, Command.PFADD, (cmd)->cmd.pfadd(key, elements),
					oneStatusConverter)
					.run(args);
		}
	}
	...
	public Status pfMerge(final byte[] destKey, final byte[]... keys) {
		...
		if(isPipeline()){
			...
			return new LettuceCommand<>(client, Command.PFMERGE, (cmd)->cmd.pfmerge(destKey, keys),
					okStatusConverter)
					.run(args);
		}
	}
