 ...
 * @author Yong.Teng
 */
public class DefaultJedisBinarySetRedisOperations<C extends BinaryJedisCommands> extends AbstractJedisBinaryRedisOperations implements JedisBinarySetRedisOperations {

	public DefaultJedisBinarySetRedisOperations(final JedisRedisClient client){
		super(client);
	}
	...
	public boolean sisMember(final byte[] key, final byte[] member){
		...
		if(isTransaction()){
			return execute((C cmd)->getTransaction().sismember(key, member).get(), ProtocolCommand.SISMEMBER, args);
		}else{
