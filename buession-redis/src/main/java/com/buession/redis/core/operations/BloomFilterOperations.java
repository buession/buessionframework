...
public interface BloomFilterOperations extends BloomFilterCommands, RedisOperations {
	...
	default Status bfAdd(final String key, final String item) {
		return doExecute((cmd)->cmd.bfAdd(KeyUtils.rawKey(this, key), item));
	}
	...
	default Long bfCard(final String key) {
		return doExecute((cmd)->cmd.bfCard(KeyUtils.rawKey(this, key)));
	}
	...
	default Boolean bfExists(final String key, final String item) {
		return doExecute((cmd)->cmd.bfExists(KeyUtils.rawKey(this, key), item));
	}
	...
	default Map<String, Number> bfInfo(final String key) {
		return doExecute((cmd)->cmd.bfInfo(KeyUtils.rawKey(this, key)));
	}
	...
	default Number bfInfo(final String key, final BfInfoOption option) {
		return doExecute((cmd)->cmd.bfInfo(KeyUtils.rawKey(this, key), option));
	}
	...
	default List<Boolean> bfInsert(final String key, final String... items) {
		return doExecute((cmd)->cmd.bfInsert(KeyUtils.rawKey(this, key), items));
	}
	...
	default List<Boolean> bfInsert(final String key, final InsertArgument argument, final String... items) {
		return doExecute((cmd)->cmd.bfInsert(KeyUtils.rawKey(this, key), argument, items));
	}
	...
	default Status bfLoadchunk(final String key, final long iterator, final byte[] data) {
		return doExecute((cmd)->cmd.bfLoadchunk(KeyUtils.rawKey(this, key), iterator, data));
	}
	...
	default List<Boolean> bfMAdd(final String key, final String... items) {
		return doExecute((cmd)->cmd.bfMAdd(KeyUtils.rawKey(this, key), items));
	}
	...
	default List<Boolean> bfMExists(final String key, final String... items) {
		return doExecute((cmd)->cmd.bfMExists(KeyUtils.rawKey(this, key), items));
	}
	...
	default Status bfReserve(final String key, final double errorRate, final long capacity) {
		return doExecute((cmd)->cmd.bfReserve(KeyUtils.rawKey(this, key), errorRate, capacity));
	}
	...
	default Status bfReserve(final String key, final double errorRate, final long capacity, final int expansion) {
		return doExecute((cmd)->cmd.bfReserve(KeyUtils.rawKey(this, key), errorRate, capacity, expansion));
	}
	...
	default Status bfReserve(final String key, final double errorRate, final long capacity, final int expansion,
		...
		return doExecute((cmd)->cmd.bfReserve(KeyUtils.rawKey(this, key), errorRate, capacity, expansion, nonScaling));
	}
	...
	default Status bfReserve(final String key, final double errorRate, final long capacity, final boolean nonScaling) {
		return doExecute((cmd)->cmd.bfReserve(KeyUtils.rawKey(this, key), errorRate, capacity, nonScaling));
	}
	...
	default Map<Long, byte[]> bfScandump(final String key, final long iterator) {
		return doExecute((cmd)->cmd.bfScandump(KeyUtils.rawKey(this, key), iterator));
	}
	...
	private <R> R doExecute(final Command.Executor<BloomFilterCommands, R> executor) {
		return execute((client)->executor.execute(client.bloomFilterCommands()));
	}
